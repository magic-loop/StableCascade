...
class WurstCore(TrainingCore, DataCore, WarpCore):
    ...
    def setup_models(self, extras: Extras) -> Models:
        ...
        if self.config.use_fsdp:
            ...
            if generator_ema is not None:
                generator_ema = FSDP(
                    generator_ema, **self.fsdp_defaults, auto_wrap_policy=fsdp_auto_wrap_policy, device_id=self.device
                )
    ...
    def setup_optimizers(self, extras: Extras, models: Models) -> TrainingCore.Optimizers:
        ...
        optimizer = self.load_optimizer(
            optimizer, "generator_optim", fsdp_model=models.generator if self.config.use_fsdp else None
        )
...
if __name__ == "__main__":
    ...
    warpcore = WurstCore(config_file_path=sys.argv[1] if len(sys.argv) > 1 else None, device=torch.device("cuda"))
