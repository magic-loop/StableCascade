...
from modules.stage_b import AttnBlock, FeedForwardBlock, ResBlock, StageB, TimestepBlock
        ...
        transforms = torchvision.transforms.Compose(
            [
                ...
                (
                    ...
                    else torchvision.transforms.CenterCrop(self.config.image_size)
                ),
            ]
        )
        ...
        if images is not None:
            ...
            if is_eval and not is_unconditional:
                ...
                effnet_embeddings = torch.zeros(
                    images.size(0), 16, effnet_height // 32, effnet_width // 32, device=self.device
                )
                if not is_eval:
    ...
    def _pyramid_noise(self, epsilon, size_range=None, levels=10, scale_mode="nearest"):
        ...
        for i in range(1, levels):
            ...
            if size_range is None or (size_range[0] <= h <= size_range[1] or size_range[0] <= w <= size_range[1]):
                ...
                epsilon = (
                    epsilon + torch.nn.functional.interpolate(offset, size=epsilon.shape[-2:], mode=scale_mode) * m
                )
