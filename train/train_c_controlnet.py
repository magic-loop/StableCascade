...
from gdf import GDF, CosineSchedule, CosineTNoiseCond, DDPMSampler, EpsilonTarget, P2LossWeight, VPScaler
from modules import (
    AttnBlock,
    ControlNet,
    ControlNetDeliverer,
...
class WurstCore(TrainingCore, DataCore, WarpCore):
    ...
    def setup_models(self, extras: Extras) -> Models:
        ...
        if self.config.controlnet_checkpoint_path is not None:
            ...
            controlnet.load_state_dict(
                ...
                if "state_dict" not in controlnet_checkpoint
                else controlnet_checkpoint["state_dict"]
            )
        ...
        if self.config.use_fsdp:
            ...
            controlnet = FSDP(
                controlnet, **self.fsdp_defaults, auto_wrap_policy=fsdp_auto_wrap_policy, device_id=self.device
            )
    ...
    def setup_optimizers(self, extras: Extras, models: Models) -> Optimizers:
        ...
        optimizer = self.load_optimizer(
            optimizer, "controlnet_optim", fsdp_model=models.controlnet if self.config.use_fsdp else None
        )
    ...
    def backward_pass(
        self, update, loss, loss_adjusted, models: Models, optimizers: Optimizers, schedulers: Schedulers
    ):
        if update:
            ...
            if self.is_main_node:
                ...
                if self.config.wandb_project is not None:
                    log_data = [
                        ...
                        + [wandb.Image(cnet_input[i])]
                        + [wandb.Image(images[i])]
                        for i in range(len(images))
                    ]
                    log_table = wandb.Table(
                        data=log_data, columns=["Captions", "Sampled", "Sampled EMA", "Cnet", "Orig"]
                    )
