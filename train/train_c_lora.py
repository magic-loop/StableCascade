...
from modules.lora import LoRA, ReToken, apply_lora, apply_retoken
...
class WurstCore(TrainingCore, DataCore, WarpCore):
    ...
    def setup_models(self, extras: Extras) -> Models:
        ...
        print(
            f"Effnet mem={torch.cuda.memory_allocated(self.device)}, max_mem={torch.cuda.max_memory_allocated(self.device)}"
        )
        ...
        print(
            f"Previewer mem={torch.cuda.memory_allocated(self.device)}, max_mem={torch.cuda.max_memory_allocated(self.device)}"
        )
        ...
        image_model = (
            CLIPVisionModelWithProjection.from_pretrained(self.config.clip_image_model_name)
            .requires_grad_(False)
            .to(dtype)
            .to(self.device)
        )
        print(
            f"Image encoder mem={torch.cuda.memory_allocated(self.device)}, max_mem={torch.cuda.max_memory_allocated(self.device)}"
        )
            ...
            if (tkn_regex.startswith("[") and tkn_regex.endswith("]")) or (
                ...
                if aggr_regex is not None:  # aggregate embeddings to provide an interesting baseline
                    ...
                    if len(aggr_tokens) > 0:
                        new_embedding = text_model.text_model.embeddings.token_embedding.weight.data[aggr_tokens].mean(
                            dim=0, keepdim=True
                        )
                ...
                text_model.text_model.embeddings.token_embedding.weight.data = torch.cat(
                    [text_model.text_model.embeddings.token_embedding.weight.data, new_embedding], dim=0
                )
        ...
        if self.config.use_fsdp:
            fsdp_auto_wrap_policy = functools.partial(
                ...
                min_num_params=3000,
                exclude_wrap_modules=[LoRA, ReToken],
            )
            generator = FSDP(
                generator,
                **self.fsdp_defaults,
    ...
    def setup_optimizers(self, extras: Extras, models: Models) -> Optimizers:
        ...
        optimizer = self.load_optimizer(
            optimizer, "lora_optim", fsdp_model=models.lora if self.config.use_fsdp else None
        )
    ...
    def forward_pass(self, data: WarpCore.Data, extras: Extras, models: Models):
        ...
        with torch.cuda.amp.autocast(dtype=torch.bfloat16):
            print(
                f"Before generate mem={torch.cuda.memory_allocated(self.device)}, max_mem={torch.cuda.max_memory_allocated(self.device)}"
            )
    ...
    def backward_pass(
        self, update, loss, loss_adjusted, models: Models, optimizers: TrainingCore.Optimizers, schedulers: Schedulers
    ):
        if update:
