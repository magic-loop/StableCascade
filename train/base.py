...
from core.data import Bucketeer, MultiFilter, MultiGetter, setup_webdataset_path
from core.utils import EXPECTED, EXPECTED_TRAIN, create_folder_if_necessary, update_weights_ema
from gdf import GDF, AdaptiveLossWeight
...
class DataCore(WarpCore):
    ...
    def webdataset_path(self):
        if isinstance(self.config.webdataset_path, str) and (
            self.config.webdataset_path.strip().startswith("pipe:")
            or self.config.webdataset_path.strip().startswith("file:")
        ):
    ...
    def webdataset_preprocessors(self, extras: Extras):
        ...
        captions_getter = MultiGetter(
            rules={("old_caption", "caption"): lambda oc, c: get_caption(json.loads(oc)["og_caption"], c, p_og=0.05)}
        )
        ...
        return [
            ...
            (
                ("txt", identity, "captions")
                if self.config.captions_getter is None
                else (self.config.captions_getter[0], eval(self.config.captions_getter[1]), "captions")
            ),
        ]

    def setup_data(self, extras: Extras, models: Models) -> WarpCore.Data:
        # SETUP DATASET
        ...
        dataset = (
            wds.WebDataset(dataset_path, resampled=True, handler=handler)
            .select(
                MultiFilter(rules={f[0]: eval(f[1]) for f in self.config.dataset_filters})
                if self.config.dataset_filters is not None
                else lambda _: True
            )
            .shuffle(690, handler=handler)
            .decode("pilrgb", handler=handler)
            .to_tuple(*[p[0] for p in preprocessors], handler=handler)
            .map_tuple(*[p[1] for p in preprocessors], handler=handler)
        ...
        if self.config.multi_aspect_ratio is not None:
            ...
            dataloader_iterator = Bucketeer(
                dataloader,
                density=self.config.image_size**2,
                factor=32,
...
class TrainingCore(DataCore, WarpCore):
    ...
    @abstractmethod
    def backward_pass(
        self, update, loss, loss_adjusted, models: Models, optimizers: Optimizers, schedulers: WarpCore.Schedulers
    ):
        raise NotImplementedError("This method needs to be overriden")
    ...
    def train(
        ...
        pbar = (
            tqdm(range(start_iter, max_iters + 1)) if self.is_main_node else range(start_iter, max_iters + 1)
        )  # <--- DDP
        ...
        for i in pbar:
            ...
            grad_norm = self.backward_pass(
                i % self.config.grad_accum_steps == 0 or i == max_iters,
                loss,
                loss_adjusted,
                models,
                optimizers,
            ...
            if (
                self.is_main_node
                and self.config.wandb_project is not None
            ...
            if i == 1 or i % (self.config.save_every * self.config.grad_accum_steps) == 0 or i == max_iters:
                ...
                if np.isnan(loss.mean().item()):
                    if self.is_main_node and self.config.wandb_project is not None:
                        ...
                        wandb.alert(
                            title=f"Skipping sampling & checkpoint for training run {self.config.wandb_run_id}",
    ...
    def save_checkpoints(self, models: Models, optimizers: Optimizers, suffix=None):
        ...
        for key in optimizers_dict:
            ...
            if optimizer is not None:
                self.save_optimizer(
                    optimizer, f"{key}_optim{suffix}", fsdp_model=models_dict[key] if self.config.use_fsdp else None
                )
        ...
        with torch.no_grad():
            ...
            if self.is_main_node:
                ...
                sampled_images_ema = torch.cat(
                    [
                        self.decode_latents(sampled_ema[i : i + 1], batch, models, extras)
