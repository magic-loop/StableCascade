...
class WarpCore(ABC):
    ...
    @abstractmethod
    def setup_data(self, extras: Extras, models: Models) -> Data:
    ...
    def setup_extras_post(
        self, extras: Extras, models: Models, optimizers: Optimizers, schedulers: Schedulers
    ) -> Extras:
        ...
        if checkpoint is not None:
            try:
                if fsdp_model is not None:
                    sharded_optimizer_state_dict = FSDP.scatter_full_optim_state_dict(  # <---- FSDP
                        (
                            checkpoint
                            if (
                                self.is_main_node
                                or self.fsdp_defaults["sharding_strategy"] == ShardingStrategy.NO_SHARD
                            )
                            else None
                        ),
                        fsdp_model,
    ...
    def save_model(self, model, model_id=None, full_path=None, is_fsdp=False):
        ...
        create_folder_if_necessary(full_path)
        print("SAVING MODEL")
        print(model["weights"][0].state_dict())
        print(model["weights"][0].size())
    ...
    def __call__(self, single_gpu=False):
        ...
        data = self.setup_data(extras, models)
        ...
        print("Removing unneeded models")
        if self.config.cache_embeddings:
            if "text_model" in models.__dict__:
                del models.__dict__["text_model"]
            print(
                f"After deleting text encoder mem={torch.cuda.memory_allocated(self.device)}, max_mem={torch.cuda.max_memory_allocated(self.device)}"
            )
