...
from ..utils import EXPECTED, EXPECTED_TRAIN, create_folder_if_necessary, update_weights_ema
...
class DiffusionCore(WarpCore):
    ...
    def forward_pass(self, data: WarpCore.Data, extras: Extras, models: Models):
        ...
        with torch.cuda.amp.autocast(dtype=torch.bfloat16):
            ...
            if self.config.gdf_target_reparametrization == TargetReparametrization.EPSILON:
                pred = extras.gdf.undiffuse(noised, logSNR, pred)[
                    1
                ]  # transform whatever prediction to epsilon to use in the loss
                target = noise
            elif self.config.gdf_target_reparametrization == TargetReparametrization.X0:
                pred = extras.gdf.undiffuse(noised, logSNR, pred)[
                    0
                ]  # transform whatever prediction to x0 to use in the loss
                target = latents
    ...
    def train(
        self, data: WarpCore.Data, extras: Extras, models: Models, optimizers: Optimizers, schedulers: Schedulers
    ):
            ...
            if i == 1 or i % (self.config.save_every * self.config.grad_accum_steps) == 0 or i == max_iters:
                ...
                if np.isnan(loss.mean().item()):
                    if self.is_main_node and self.config.wandb_project is not None:
                        ...
                        wandb.alert(
                            title=f"Skipping sampling & checkpoint for training run {self.config.run_id}",
    ...
    def save_checkpoints(self, models: Models, optimizers: Optimizers, suffix=None):
        ...
        for key in optimizers_dict:
            ...
            if optimizer is not None:
                self.save_optimizer(
                    optimizer, f"{key}_optim{suffix}", fsdp_model=models.generator if self.config.use_fsdp else None
                )
